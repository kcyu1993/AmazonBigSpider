/*
Modified by Kaicheng Yu 2017.12.12
Version 0.1
	Add the ImagePool() to add the image url + Asin into redis, for spider.
...
func getimageurls() []map[string]interface{} {
	num, e := RedisClient.Llen(MyConfig.ImageUrlpool)
	if e != nil {
		panic(e)
	}
	if num > 0 {
		AmazonImageLog.Logf("Imageurlpool exist Today , %s with number %d",  MyConfig.ImageUrlpool, num)
		//panic("Imageurlpool has exist today with number " + util.IS(int(num)))
		for i := int64(0); i < num; i ++ {
			// Remove all the results
			RedisClient.Lpop(MyConfig.ImageUrlpool)
			RedisClient.Lpop(MyConfig.ImageUrlhashpool)
			RedisClient.Lpop(MyConfig.ImageUrldealpool)
		}
	}
	result, err := DataDb.Select(strings.Replace(MyConfig.Imagesql, "{?}", Today, -1))
	if err != nil {
		panic(err)
	}
	return result
...
func sentimagetoredis(results []map[string] interface{}) {

	for _, result := range results {
		imageId := result["id"].(string)
		// Replace the image url here, when add into the Redis
		//imageUrl := replaceImageUrlWithFormat(MyConfig.ImageUrlFormat, result["img"].(string))
		imageUrl := result["img"].(string)
		// ImageURL|Asin
		_, err := RedisClient.Lpush(MyConfig.ImageUrlpool, imageUrl + "|" + imageId)
		if err != nil {
			fmt.Printf("error:%v,%v\n", imageUrl, err)
		} else {
			fmt.Println("-")
